...
if PYSIDE6_AVAILABLE:
    ...
    class ImportReviewDialog(QDialog):
        ...
        Jedine import okno (nahradilo drivejsi ImportDialog a
        MultiImportResultsDialog). Leva strana (seznam, hromadny vyber, filtr) se u
    ...
    class PreferencesDialog(QDialog):
        ...
        def _build_ui(self) -> None:
            ...
            # Vysledek "Test AI" hned pod modelem, kde se provider/model nastavuje.
            self.ai_test_result_label = QLabel()
            form.addWidget(QLabel("AI test"), 7, 0)
            form.addWidget(self.ai_test_result_label, 7, 1, 1, 3)
            form.addWidget(QLabel("EPUB text limit"), 8, 0)
            self.ai_text_limit_edit = QLineEdit(str(ai_settings["text_limit"]))
            form.addWidget(self.ai_text_limit_edit, 8, 1, 1, 3)
            form.addWidget(QLabel("AI timeout (s)"), 9, 0)
            self.ai_timeout_edit = QLineEdit(str(ai_settings["timeout"]))
            form.addWidget(self.ai_timeout_edit, 9, 1, 1, 3)
            form.addWidget(QLabel("Knih naráz (multiimport)"), 10, 0)
            ...
            form.addWidget(self.ai_workers_spin, 10, 1)
            workers_hint = QLabel("doporučeno 5")
            workers_hint.setEnabled(False)
            form.addWidget(workers_hint, 10, 2, 1, 2)
    ...
    class CalibreMetaQtWindow(QMainWindow):
        ...
        def start_epub_import(
            ...
            *,
            ...
            """Spusti analyzu EPUB importu a po dokonceni otevre ImportReviewDialog.
